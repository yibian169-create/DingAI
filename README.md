# 得应盯 · 企业官网 AI GEO CMS 建站系统

> 一个后台，管理整站内容、品牌与前端展示 —— 栏目/文章/产品、图片空间、全国分站、主题设置、AI GEO、首页可视化编辑，零框架依赖、宝塔一键部署。
...
> **GEO = 生成式引擎优化（Generative Engine Optimization）**：当客户用 DeepSeek、豆包、Kimi、文心、ChatGPT、Perplexity 提问时，让你的官网出现在它们的答案 / 推荐里 —— 抓住 AI 搜索带来的新流量，而不只是传统搜索引擎。

![GEO 中心](assets/geo.svg)

三大能力，一个后台全包：

| 能力 | 后台能做什么 |
| --- | --- |
| **GEO 体检** | 综合健康度评分（0-100），逐项体检文章问答卡、标题关键词、实体一致性、问答覆盖率，红黄绿提示缺哪补哪 |
| **GEO 内容生成** | 商家介绍 + 客户问题词蒸馏 + 品牌资料库；批量生成"问题→标准答案"问答资产，一键变文章并自动带 FAQ 卡片 |
| **GEO 监控** | AI 引用率趋势、各平台引用明细、竞品对比监控、负面词预警，随时掌握网站在 AI 答案里的存在感 |

工作流：全站诊断 → 生成问答（结合品牌库）→ 做 FAQ 卡片 → 分发并监控 AI 是否引用你。

| 功能 | 能做什么 |
| --- | --- |
| **🌐 GEO 中心（核心卖点）** | 让 DeepSeek、豆包、Kimi、文心、ChatGPT 等 AI 在回答客户问题时**优先推荐你的网站**：全站 GEO 体检打分、批量生成"客户问题→标准答案"问答库、自动 FAQ 卡片、品牌资料库、AI 引用率 / 竞品 / 负面监控 |
